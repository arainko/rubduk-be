...
import zio.test.{DefaultRunnableSpec, ZSpec, testM}
...
object ServiceTest extends DefaultRunnableSpec {
  ...
  val cos = testM("repo test") {
    for {
      id <- UserRepository.insert(UserDAO(
        None, "Test", None, "asdads", LocalDate.parse("2020-01-01").some, OffsetDateTime.parse("2020-11-22T22:06:08.211767+01")
      ))
